...
/* ---- Buffer circular de impresión: MAX_BUFFERS entradas de 100 chars ---- */
char buf [MAX_BUFFERS] [100];
/* ---- Índice de escritura y de lectura (spooler) sobre el buffer circular ---- */
int buffer_index;
int buffer_print_index;

/* ---- Sincronización: mutex y condiciones para productores/spooler ---- */
pthread_mutex_t buf_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t buf_cond = PTHREAD_COND_INITIALIZER;
pthread_cond_t spool_cond = PTHREAD_COND_INITIALIZER;
/* ---- Contabilidad: buffers libres y líneas pendientes por imprimir ---- */
...
int main (int argc, char **argv){
    ...
    /* ---- Variables auxiliares: i para bucles, r para códigos de retorno ---- */
    int i, r;

    /* ---- Inicializar índices del buffer circular ---- */
    buffer_index = buffer_print_index = 0;
    /* ---- Crear hilo spooler (consumidor/imprimiendo líneas) ---- */
    if ((r = pthread_create (&tid_spooler, NULL, spooler, NULL)) != 0) {
        /* ---- Reportar error si falla la creación del spooler ---- */
        fprintf (stderr, "Error = %d (%s)\n", r, strerror (r)); exit (1);
    }
    /* ---- Identificadores lógicos para productores (0..9) ---- */
    int thread_no [10];
    ...
    for (i = 0; i < 10; i++) {
        ...
        if ((r = pthread_create (&tid_producer [i], NULL, producer, (void *) &thread_no [i])) != 0) {
            /* ---- Reportar error si falla la creación de algún productor ---- */
            fprintf (stderr, "Error = %d (%s)\n", r, strerror (r)); exit (1);
        }
    }
    ...
    for (i = 0; i < 10; i++)
        if ((r = pthread_join (tid_producer [i], NULL)) == -1) {
            /* ---- Reportar error si falla el join de algún productor ---- */
            fprintf (stderr, "Error = %d (%s)\n", r, strerror (r)); exit (1);
        }
    
    /* ---- Esperar a que el spooler vacíe todas las líneas pendientes ---- */
    while (lines_to_print) sleep (1);
    /* ---- Cancelar el hilo spooler (ya no hay trabajo) ---- */
    if ((r = pthread_cancel (tid_spooler)) != 0) {
        /* ---- Reportar error si falla la cancelación del spooler ---- */
        fprintf (stderr, "Error = %d (%s)\n", r, strerror (r)); exit (1);
    }

    /* ---- Finalización correcta del proceso principal ---- */
    exit (0);
}
